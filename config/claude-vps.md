...
> Legacy VPS instruction template from the older Claude-first baseline.
> Repo-level `AGENTS.md` is now canonical for project policy and migration direction.
> Do not treat this file as the source of truth for the AgentGLS architecture.
...
- `bash /opt/agentgls/scripts/memory.sh get <id>` — full detail for a session (use first 8 chars of ID)
- `bash /opt/agentgls/scripts/memory.sh search "<query>"` — search session summaries by keyword
- `bash /opt/agentgls/scripts/memory.sh deep-search "<query>"` — search detailed summaries
- `bash /opt/agentgls/scripts/memory.sh full-search "<query>"` — search raw session content (slow)
- `bash /opt/agentgls/scripts/memory.sh add-memory '<json>'` — store a structured memory
- `bash /opt/agentgls/scripts/memory.sh add-profile '<json>'` — store user profile data
...
- `bash /opt/agentgls/scripts/tasks.sh list` — list all tasks
- `bash /opt/agentgls/scripts/tasks.sh add '<json>'` — create a task
- `bash /opt/agentgls/scripts/tasks.sh remove <id>` — delete a task (first 8 chars of ID)
- `bash /opt/agentgls/scripts/tasks.sh pause <id>` — disable without deleting
- `bash /opt/agentgls/scripts/tasks.sh resume <id>` — re-enable a paused task
- `bash /opt/agentgls/scripts/tasks.sh run <id>` — run immediately
- `bash /opt/agentgls/scripts/tasks.sh history` — recent activity across all tasks
- `bash /opt/agentgls/scripts/tasks.sh history <id>` — history for a specific task
- `bash /opt/agentgls/scripts/tasks.sh sync` — regenerate cron entries from DB
...
Example:
```bash
bash /opt/agentgls/scripts/tasks.sh add '{"name":"Cat joke","cron_expr":"0 * * * *","prompt":"Tell me a short, funny cat joke","chat_id":"6599988942"}'
```
...
- Logs: `/opt/agentgls/logs/`
- Scripts: `/opt/agentgls/scripts/`
